...
describe('EventsList.vue', () => {
  ...
  it('should render appropriate amount of events', () => {
    expect(
      vm.$el.querySelectorAll('.card').length,
    ).to.equal(3);
  });
  ...
  it('should render appropriate amount of event cards', () => {
    expect(
      vm.$el.querySelectorAll('.mdl-card__actions').length,
    ).to.equal(2);
  });

  it('Konkurs - should render <a> tag with proper href ', () => {
    expect(
      vm.$el.querySelectorAll('.mdl-button')[0].href,
    ).to.contain(`/${events[0].seoSlug}/konkurs`);
  });

  it('Agenda - should render <a> tag with proper href ', () => {
    expect(
      vm.$el.querySelectorAll('.mdl-button')[1].href,
    ).to.contain(`/event/${events[0].seoSlug}`);
  });
});
