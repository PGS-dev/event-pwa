...
describe('AdminPage.vue', () => {
  ...
  it('should render appropriate amount of events', () => {
    expect(
      vm.$el.querySelectorAll('tbody tr').length,
    ).to.equal(3);
  });

  it('should render proper title', () => {
    expect(
      vm.$el.querySelectorAll('tbody tr')[0]
  ...
  it('Konkurs - should render <a> tag with proper href ', () => {
    expect(
      vm.$el.querySelectorAll('a.mdl-button')[0].href,
    ).to.contain(`/admin/${events[0].seoSlug}/konkurs`);
  });
});
